...
class Cache {
  ...
  Cache init(int maxSeconds, int maxSize) {
    ...
    if (maxSeconds > 0) {
      logger.atInfo().log("Initialize Cache with maxSeconds=" + maxSeconds + " maxSize=" + maxSize);
      dbCache =
  ...
  OwnersDb get(
    ...
    if (dbCache == null) { // Do not cache OwnersDb
      logger.atInfo().log("Create new OwnersDb, key=" + key);
      return new OwnersDb(
    ...
    try {
      ...
      logger.atSevere().log("Cache.get has exception for " + Config.getChangeId(changeData), e);
      return new OwnersDb(
          projectState, accountCache, emails, key, repository, changeData, branch, files);
    }
  }
