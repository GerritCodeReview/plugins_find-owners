...
class Cache {
  ...
  Cache init(int maxSeconds, int maxSize) {
    ...
    if (maxSeconds > 0) {
      logger.atInfo().log("Initialize Cache with maxSeconds=%s maxSize=%s", maxSeconds, maxSize);
      dbCache =
  ...
  OwnersDb get(
    ...
    if (dbCache == null) { // Do not cache OwnersDb
      logger.atInfo().log("Create new OwnersDb, key=%s", key);
      return new OwnersDb(
          projectState, accountCache, emails, key, repository, changeData, branch, files);
    }
    try {
      logger.atInfo().log("Get from cash %s, key=%s, cache size=%s", dbCache, key, dbCache.size());
      return dbCache.get(
          key,
