...
class Cache {
  ...
  Cache init(int maxSeconds, int maxSize) {
    ...
    if (maxSeconds > 0) {
      logger.atInfo().atMostEvery(5, TimeUnit.MINUTES).log(
          "Initialize Cache with maxSeconds=" + maxSeconds + " maxSize=" + maxSize);
      dbCache =
      ...
      logger.atInfo().atMostEvery(5, TimeUnit.MINUTES).log("Cache disabled.");
      dbCache = null;
    }
    return this;
  }
  ...
  OwnersDb get(
    ...
    if (dbCache == null) { // Do not cache OwnersDb
      logger.atInfo().atMostEvery(5, TimeUnit.MINUTES).log("Create new OwnersDb, key=" + key);
      return new OwnersDb(
          projectState, accountCache, emails, key, repository, changeData, branch, files);
    }
    try {
      logger.atInfo().atMostEvery(5, TimeUnit.MINUTES).log(
          "Get from cash " + dbCache + ", key=" + key + ", cache size=" + dbCache.size());
      return dbCache.get(
          key,
      ...
      logger.atSevere().atMostEvery(5, TimeUnit.MINUTES).log(
          "Cache.get has exception for " + Config.getChangeId(changeData), e);
      return new OwnersDb(
          projectState, accountCache, emails, key, repository, changeData, branch, files);
    }
  }
