...
public class OwnersFileRule implements SubmitRule {
  ...
  public Collection<SubmitRecord> evaluate(ChangeData cd, SubmitRuleOptions options) {
    ...
    try {
      pluginConfig = pluginConfigFactory
          .getFromProjectConfigWithInheritance(cd.project(), pluginName);
    } catch (NoSuchProjectException e) {
      logger.atSevere().withCause(e).log("Error when fetching config of change {}'s project", cd.getId(), e);
      return Collections.emptyList();
    }
    ProjectConfig projectConfig = projectConfigFactory.create(cd.project());
