...
(function (window) {
  ...
  Polymer({
    ...
    properties: {
      ...
      // all groups on files map to owners.
      _fileGroups: {
        type: Array,
        value() {
          return [];
      ...
      // changes made by the user, additions and deletions.
      _reviewerChanges: {
        type: Object,
        value() {
          return {};
    ...
    _populateFileGroups(ownersResponse) {
      ...
      // Since checkbox on same owner should always have same state
      // Here collects all checkboxes, so once state changed,
      // we can propagate to all boxes
      this._fileGroups.forEach((group, i) => {
        group.sections.forEach((section, j) => {
          section.owners.forEach((owner, k) => {
            this._ownerStatusMap[owner.email].checkboxes.push([
              '_fileGroups',
              i,
              'sections',
              j,
    ...
    _checkboxChanges($event) {
      ...
      this._ownerStatusMap[owner.email].checkboxes.forEach(checkbox => {
  ...
  Gerrit.install((plugin) => {
    ...
    // Leaving page should remove element as well.
