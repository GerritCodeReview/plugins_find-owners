...
Gerrit.install(function(self) {
  ...
  function popupFindOwnersPage(context, change, revision, onSubmit) {
    ...
    const BUTTON_STYLE = Gerrit.css(
        ...
        'cursor: pointer;');
    const getElWithText = (tagName, text) => {
    ...
    const getBulletEl = () => {
      return getElWithText('small', '\u2605');  // a Black Star
    };
    const getIsExemptedEl = () => {
      return getElWithText('b', 'This change is Exempt-From-Owner-Approval.');
    };

    const getNoOwnerEl = () => {
      return getElWithText('b', 'No owner was found for changed files.');
    };

    const getOnSubmitHeader = () => {
      return getElWithText(
          'b', 'WARNING: Need owner Code-Review vote before submit.');
    };

    const getOwnersHeaderEl = () => {
      return getElWithText('b', 'Owners in alphabetical order:');
    };

    const getSelectReviewersEl = () => {
      const el = document.createElement('p');
      el.appendChild(getElWithText(
          'b',
          'Check the box before owner names to select reviewers, ' +
              'then click the "Apply" button.'));
      el.appendChild(getElWithText('br'));
      const smallEl = getElWithText(
          'small',
          'If owner-approval requirement is enabled, ' +
              ...
              '(Or declare "');
      const smallerEl = getElWithText('b', 'Exempt-From-Owner-Approval:');
      smallerEl.style.fontSize = '80%';
      smallEl.appendChild(smallerEl);
      smallEl.appendChild(getElWithText('i', 'reasons...'));
    ...
    const getGroupTypeHeaderEl = (text) => {
      const el = getElWithText('span');
      el.appendChild(getElWithText('hr'));
      el.appendChild(getElWithText('b', text));
      el.appendChild(getElWithText('br'));
      return el;
    };
    const HTML_GROUP_TYPE_HEADER = [
      getGroupTypeHeaderEl(
      ...
      getGroupTypeHeaderEl('Files with +1 or +2 Code-Review vote from owners:'),
      getGroupTypeHeaderEl('Files without any named owner:'),
    ];
    ...
    function showBoldKeyValueLines(args, key, value) {
      args.push(
          getElWithText('hr'), strElement('<b>' + key + '</b>:'),
          getElWithText('br'));
      value.split('\n').forEach(function(line) {
        args.push(strElement(line), getElWithText('br'));
      });
    }
    ...
    function showFilesAndOwners(result, args) {
      ...
      function colorSpan(str, color) {
        const el = getElWithText('span', str)
      ...
      function addGroupsToDiv(div, keys, titleEl) {
        ...
        function addOwner(itemDiv, ownerEmail) {
          ...
          itemDiv.appendChild(getElWithText('span', '\u00a0\u00a0'));
        ...
        keys.forEach(function(key) {
          ...
          var itemEl = getElWithText('span');
          itemEl.appendChild(getBulletEl());
          itemEl.appendChild(document.createTextNode('\u00a0'));
          itemEl.appendChild(getElWithText('b', key));
          itemEl.appendChild(document.createTextNode(
          ...
          itemDiv.appendChild(getElWithText('br'));
      ...
      function addOwnersDiv(div, titleEl) {
        ...
        div.style.display = 'inline';
        div.appendChild(titleEl);
        div.appendChild(getElWithText('br'));
        ...
        result.owners.sort(compareOwnerInfo).forEach(function(ownerInfo) {
          ...
          var emailEl = getElWithText('span', email);
          if (email != '*') {  // do not list special email *
            ...
            const spanEl = getElWithText('span');
            spanEl.appendChild(document.createTextNode('\u00a0\u00a0'));
            spanEl.appendChild(emailEl);
            spanEl.appendChild(getElWithText('br'));
      ...
      function updateDivContent() {
        ...
        if (isExemptedFromOwnerApproval()) {
          headerMessageDiv.appendChild(getIsExemptedEl());
        } else {
          if (onSubmit) {
            headerMessageDiv.appendChild(getOnSubmitHeader());
          }
          headerMessageDiv.appendChild(getSelectReviewersEl());
        }
        ...
        addOwnersDiv(ownersDiv, getOwnersHeaderEl());
      }
    ...
    function showFindOwnersResults(result) {
      function prepareElements() {
        var elems = [];
        var textEl =
            Object.keys(result.file2owners).length <= 0 ? getNoOwnerEl() : null;
        useContextPopup = !!context && !!textEl && !!context.popup;
        if (!!textEl) {
          if (useContextPopup) {
            elems.push(getElWithText('hr'), textEl, getElWithText('hr'));
            var onClick = function() {
              context.hide();
            };
            elems.push(
                context.button('OK', {onclick: onClick}), getElWithText('hr'));
          } else {
